...
File name: syn-cheng.py

Description: Code to compare with Cheng et al. (2020)
...
def get_theta(number_dimensions,use_t_in_e,number_environments):
  ...
  theta4 = np.random.uniform(1.0,2.0,(number_dimensions*2+2,1))
...
def get_data(u1,u2,u3,u4,theta1,theta2,theta3,theta4,theta5,theta6,number_observations,number_dimensions,use_t_in_e,number_environments):
  ...
  y = np.dot(np.concatenate((x1,x2,u4,t),axis=1),theta4) + noise
  y_0 = np.dot(np.concatenate((x1,x2,u4,zeros),axis=1),theta4) + noise
  y_1 = np.dot(np.concatenate((x1,x2,u4,ones),axis=1),theta4) + noise
...
def main(args):
  ...
  np.savetxt('syn-cheng/pvalue_empty.csv', pvalue_empty, delimiter=",")
  np.savetxt('syn-cheng/pvalue_x2.csv', pvalue_x2, delimiter=",")
  np.savetxt('syn-cheng/pvalue_x3.csv', pvalue_x3, delimiter=",")
  np.savetxt('syn-cheng/pvalue_x2x3.csv', pvalue_x2x3, delimiter=",")
